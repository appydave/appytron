 ...
 */
/**
 * Redirect ALL Electron-owned persistence when `APPYTRON_HOME` is set.
 *
 * Why this exists: an agent-driven acceptance run drives the REAL UI, and the
 * real UI writes to the real data directory. Without this, a run mutates the
 * user's live `userData` — settings, counters, caches — while claiming to be
 * isolated. Driving the real UI is not negotiable; pointing it at real user
 * data is.
 *
 * `pictures` is redirected too because scaffolded apps commonly put their
 * visible output root under it.
 *
 ...
 *
 * Unset in a normal launch, so production behaviour is byte-identical. An app
 * with its OWN service/home env var must still set that separately — this
 * covers only what Electron owns.
 *
...
function redirectHomeIfRequested(logger: Logger): void {
  ...
  logger.warn(
    `APPYTRON_HOME=${home}: userData and pictures redirected — isolated run, ` +
